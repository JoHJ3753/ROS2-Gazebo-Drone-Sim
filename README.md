...
| 런타임 상대이동 연결 | 코드 연결 완료 | 속도 지정 없는 단일 상대이동 지원 |
| 런타임 상대회전 연결 | 코드 연결 완료 | 속도 지정 없는 단일 180도 미만 회전 지원 |
| 런타임 호버링 연결 | 코드 연결 완료 | 무기한 또는 시간 지정 단일 호버링 지원 |
| 런타임 자동 착륙 | 코드 연결 완료 | PX4 착륙 및 Disarm 상태 확인 |
| 런타임 명령 취소 | 코드 연결 완료 | 진행 중인 이동·회전·시간 지정 호버링과 대기 중 이륙 취소 |
| LLM과 PX4 종단 간 연동 | 통합 검증 대기 | 파서와 명령 토픽 연결 완료, 자연어 비행 검증 필요 |
...
현재 실제 런타임 PX4 연결이 완료된 명령은 `takeoff`, `land`,
`move_drone`, `rotate_relative`, `hover`, `cancel`입니다.
나머지 명령은 단계적으로 연결하고 있습니다.
...
현재 런타임에서는 단일 `takeoff`, `land`, `move_drone`,
`rotate_relative`, `hover`, `cancel` 명령만 연결되어 있습니다.
`hover`는 다음 명령까지 유지하거나 `duration_s`로 시간을 지정할 수
있습니다. `cancel`은 진행 중인 이동·회전·시간 지정 호버링 또는 대기
중인 이륙을 중단합니다.
이동 속도(`speed_mps`)와 회전 속도(`yaw_speed_dps`) 지정은 무시하지 않고 거부합니다.
회전각의 절댓값이 180도 이상이면 목표 방향 정규화 때문에 요청한
회전 경로를 보장할 수 없어 거부합니다. 복합 명령도 발행하지 않습니다.
...
- [x] JSON 기반 `rotate_relative` 런타임 연결 (속도 지정 및 180도 이상 제외)
- [x] JSON 기반 `hover` 런타임 연결
- [x] JSON 기반 `land` 및 자동 Disarm 처리
- [x] JSON 기반 `cancel` 런타임 연결
